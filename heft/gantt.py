...
def showGanttChart(proc_schedules):
    ...
    for idx, proc in enumerate(processors):
        for job in proc_schedules[proc]:
            ...
            ax.text(0.5 * (job.start + job.end - len(str(job.task))-0.25), (idx*0.5)+0.5 - 0.03125, str(int(job.wf_id)) + "_" + str(int(job.task)), color=color_choices[((job.task) // 10) % 5], fontweight='bold', fontsize=18, alpha=0.75)
